class_name DictionaryHacker
extends ToolWindow
## Dictionary hacker — fast cracker for low-security nodes (1–3).
## Fails immediately on security 4+. Satisfies CRACK_NODE objective.

enum State { IDLE, FIREWALL_LOCKED, READY, CRACKING, SUCCESS, FAILED, INCOMPATIBLE }
...
func _start_crack() -> void:
	...
	EventBus.tool_task_started.emit("dictionary_hacker", NetworkSim.connected_node_id)
	EventBus.log_message.emit(
		"Dictionary attack initiated on %s" % data.get("ip", "?"), "info"
	)
	_update_ui()
...
func _abort_crack() -> void:
	...
	EventBus.tool_task_completed.emit("dictionary_hacker", NetworkSim.connected_node_id, false)
	EventBus.log_message.emit("Dictionary attack aborted.", "warn")
	_update_ui()
...
func _on_crack_complete() -> void:
	...
	EventBus.tool_task_completed.emit("dictionary_hacker", node_id, true)
	_update_ui()
...
func _update_ui() -> void:
	match _state:
		...
		State.INCOMPATIBLE:
			var data: Dictionary = NetworkSim.get_node_data(NetworkSim.connected_node_id)
			status_label.text = "INCOMPATIBLE:  %s  —  security too high for dictionary attack" % data.get("ip", "?")
			status_label.add_theme_color_override("font_color", Color(1.0, 0.08, 0.55))
			action_btn.text     = "INCOMPATIBLE"
			action_btn.disabled = true
...
func _setup_theme() -> void:
	...
	crack_fill.bg_color = Color(1.0, 0.75, 0.0)
	...
	crack_pct.add_theme_color_override("font_color", Color(0.65, 0.55, 0.25))
	action_btn.add_theme_color_override("font_color", Color(1.0, 0.75, 0.0))


func _crack_time(security: int) -> float:
	return maxf(3.0, float(security) * 5.0)
